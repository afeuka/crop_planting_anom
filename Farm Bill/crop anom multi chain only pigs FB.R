...
if(!file.exists(paste0("./Model outputs/",subfolder))){
  dir.create(paste0("./Model outputs/",subfolder))
  dir.create(paste0("./Model outputs/",subfolder,"/Plots"))
}
...
for(commod_idx in 1:length(commod_names_c)){
  if(commod_idx!=4){ #not enough peanut FB counties
    ...
    # if("take.hog.intens.5yeartrend.sc"%in%colnames(dat_clean_op)){
    #   subfolder <- "Take Trend"
    # } else if("take.hog.intens.prev.sc"%in%colnames(dat_clean_op)) {
